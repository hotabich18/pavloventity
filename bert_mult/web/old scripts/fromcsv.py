...
def getcountnews():
    with open('lenta-ru-news.csv', newline='') as File:
...
def insertRow(userword, wordtype, con):
    query_template = "INSERT INTO words1 (word,type) VALUES ('{word}', '{type}')"
...
if __name__ == '__main__':
    ...
    cur.execute("PREPARE insuser AS " +
                "INSERT INTO words1 (word, type) VALUES ($1, $2)")
    ...
    with open('lenta-ru-news.csv', newline='') as File:
        ...
        for row in reader:
            id = 0
            try:
                data = getEntity(row[2])
                for entity in data[0][0]:
                    insertRow(entity, data[0][1][id], con)
                    id = id + 1
                count = count + 1
